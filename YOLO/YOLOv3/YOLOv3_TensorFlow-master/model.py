...
class yolov3(object):
	...
	# 输出三个scale layers上的feature_map:
	# feature_map1:1*13*13*255, feature_map2=1*26*26*255, feature_map3=1*52*52*255
	...
	# 输入：一个scale上的featuremap,就是forward函数输出值之一，在这个anchors上的三个anchors的尺寸
	# 输出：x_y_offset：[*, *, 1, 2] cell的相对坐标，boxes: [N, *, *, 3, 4] 在原始图像上的绝对坐标，
	# 输出：conf_logits: [N, 13, 13, 3, 1] 每一个box的置信度，prob_logits: [N, 13, 13, 3, class_num] 每个上面的类别属性
    def reorg_layer(self, feature_map, anchors):
        ...
        ratio = tf.cast(self.img_size / grid_size, tf.float32) # 一个len=2的列表，[32,32],[16,16],[8,8]
        # rescale the anchors to the feature_map
        # NOTE: the anchor is in [w, h] format!
        rescaled_anchors = [(anchor[0] / ratio[1], anchor[1] / ratio[0]) for anchor in anchors]
		#这一列是进行升维吧，[N, 13, 13, 3, 85]，为后面的prediction进行准备。
        ...
        box_centers = tf.nn.sigmoid(box_centers)  # 见YOLOv2，bound the gound truth  to fall between 0 and 1.
		...
		# 这里有必要说一下，YOLOv3是对每一个box对应一个anchor的，我以为这里是每一个box对应所有的所有的anchor的，改进想法？
        box_sizes = tf.clip_by_value(tf.exp(box_sizes), 1e-9, 50) * rescaled_anchors
        # rescale to the original image scale
        box_sizes = box_sizes * ratio[::-1]  # box_sizes: [N, 13, 13, 3, 2]
	...
	# 对每个尺度上的位置，置信度和分类结果进行预测
	# 输入：forward函数的输出值，
	# 输出：boxes：[N, (13*13+26*26+52*52)*3, 4],将每个尺度上的预测结果融合，注意4现在知原始图像上x_min, y_min, x_max, y_max
	# 输出：confs: [N, (13*13+26*26+52*52)*3, 1],所有预测的框的置信度
	# 输出：probs: [N, (13*13+26*26+52*52)*3, class_num],每个预测框分类结果。
    def predict(self, feature_maps):
        ...
        for result in reorg_results:
            ...
            confs = tf.sigmoid(conf_logits) # 这就是YOLOv3中的predict an objectnes score for each bounding box using logistic regression.
            probs = tf.sigmoid(prob_logits) # 在YOLOv2中使用的是softmax，但是softmax,但是这里只是使用logistic classifiers.
		...
		# 计算每一个预测boxes的四个最大最小坐标
        center_x, center_y, width, height = tf.split(boxes, [1, 1, 1, 1], axis=-1)
        x_min = center_x - width / 2
        y_min = center_y - height / 2
        x_max = center_x + width / 2
        y_max = center_y + height / 2
		# 注意这里的boxes的维度仍是[N, (13*13+26*26+52*52)*3, 4]
