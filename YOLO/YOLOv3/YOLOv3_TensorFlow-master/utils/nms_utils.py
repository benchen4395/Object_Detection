...
# 对获得边界框，概率值和object class进行非极大值抑制
# 输出：boxes 列表，保存下来的每个bounding box的坐标
# 输出：score 列表，保存下来的概率值
# 输出：label 列表，筛选后的对应边框
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.5, iou_thresh=0.5):
    ...
    mask = tf.greater_equal(score, tf.constant(score_thresh))  # [True if score >= score_thresh else False]
    print(mask.shape), print(boxes.shape)
    # Step 2: Do non_max_suppression for each class
    for i in range(num_classes):
        # Step 3: Apply the mask to scores, boxes and pick them out
        filter_boxes = tf.boolean_mask(boxes, mask[:,i]) # 输出objectness socre大于0.5的种类的坐标
        filter_score = tf.boolean_mask(score[:,i], mask[:,i])
		# 非极大值抑制，返回的是selected_indices:表示的是一个1-D的整数张量，大小为[M]，
		# 代表的是选出来的留下来的边框下标，M小于等于max_outpuy_size.
        nms_indices = tf.image.non_max_suppression(boxes=filter_boxes,
                                                   scores=filter_score,
                                                   max_output_size=max_boxes,
                                                   iou_threshold=iou_thresh, name='nms_indices')
		# 这里是将每个class类下的对应索引标识出来
        label_list.append(tf.ones_like(tf.gather(filter_score, nms_indices), 'int32')*i)
		# tf.gather可以从获得索引下标中获得边框的具体位置
